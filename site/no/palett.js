...
(function(){
  var PALETTE_DATA={
    ...
    // Visningsnavn utad: «Sand» (ikke «Krem» — det ville kollidert med «Krem & Gull» i lista;
    // nå stemmer nøkkel og visningsnavn). accentDark snur
    // aksenten til krem i mørk modus, ellers ville espresso-aksenten forsvunnet mot mørk bg.
    sand:     {light:'#efe7d8',accent:'#38312a',dark:'#2a241e',accentDark:'#efe7d8'},
  };
  ...
  function buildPalette(key,mode){
    ...
    if(dk&&lum(bg)>lum(ink)){var t=bg;bg=ink;ink=t;}
    // accentDark er VALGFRI: paletter uten nøkkelen bruker accent i begge moduser (bit-identisk
    // med før). Data styrer, ingen navnesjekk. accentInk avledes av `accent`, så den følger
    // automatisk når accentDark slår inn i mørk modus. MÅ holdes i synk med backend/fyll.cjs.
    var accent=(dk&&p.accentDark)?p.accentDark:p.accent;
